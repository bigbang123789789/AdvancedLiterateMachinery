...
class MultiPairsGeometricHead(nn.Module):
    ...
    def forward(self, hidden_states, logits_prev, block_mask):
        ...
        # max_valid = torch.min((1 - negative_idx_pred).sum(1).max().long(), torch.as_tensor(self.max_rel_enc)).item()
        max_valid = torch.min((1 - negative_idx_pred).sum(1).max().long(), torch.as_tensor(self.max_rel_enc))
